...
/**
 * capture-toc.js
 * Screenshots the Table of Contents slide (slide 3) from every active tech-talk deck.
 * Output: captures/<deck>-toc.png
 *
 * Usage:
 *   node scripts/capture-toc.js            # all decks
 *   node scripts/capture-toc.js copilot-cli # one deck
 */
...
async function captureToc(slug) {
  console.log(`\n=== ${slug} ===`);
  killPort(PORT);
  await new Promise((r) => setTimeout(r, 1000));

  const child = spawn("cmd", ["/c", `npm run dev -- tech-talks/${slug}.md --port ${PORT}`],
    { cwd: SLIDES_DIR, detached: true, stdio: "ignore" });
  child.unref();

  console.log(`  Waiting for server...`);
  const ready = await waitForPort(PORT, READY_WAIT_MS);
  if (!ready) { console.log(`  TIMEOUT`); killPort(PORT); return false; }
  ...
  let ok = false;
  try {
    await page.goto(`http://localhost:${PORT}/${TOC_SLIDE}`, { waitUntil: "networkidle", timeout: 15000 });
    await page.waitForTimeout(1500);
    await page.evaluate(() => {
      for (const sel of [".slidev-nav", ".slidev-progress", "[class*='slidev-controls']", ".nav-return"])
        document.querySelectorAll(sel).forEach((el) => (el.style.display = "none"));
    });
    const outPath = path.join(OUTPUT_DIR, `${slug}-toc.png`);
    await page.screenshot({ path: outPath, fullPage: false });
    console.log(`  Saved: ${slug}-toc.png`);
    ok = true;
  } catch (e) {
...
async function main() {
  ...
  for (const slug of decks) {
    const ok = await captureToc(slug);
