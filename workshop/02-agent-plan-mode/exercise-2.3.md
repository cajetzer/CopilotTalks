...
It's 10:30 AM. Marcus has just shared his plan output with the group. David reads through it and nods — the stat cards are well-designed. Then he spots the gap: *the counters can show counts, but there's no way to add data without opening a database client.* He wants an "Add to Universe" form panel right on the homepage, so participants can type in a character name or location and watch the counter tick up in real time.
